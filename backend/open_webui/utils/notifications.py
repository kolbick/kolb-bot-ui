...
async def test_target(user_id: str, target_id: str, app_name: str = 'Kolb-Bot') -> dict[str, Any]:
    notifications = await _load_notifications(user_id)
    ...
    await _send_webhook(
        app_name,
        target,
        'This is a test notification from Kolb-Bot.',
...
async def dispatch_notification_event(app: Any, event: Any) -> None:
    ...
    app_name = getattr(getattr(app, 'state', None), 'WEBUI_NAME', 'Kolb-Bot')
